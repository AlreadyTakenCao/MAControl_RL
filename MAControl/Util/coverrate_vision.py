...
def calculate_coverage(num, step, loop=0):
    ...
    open(pardir + '/MAControl-AFIT/cover_rate_Folder' + txt_name, 'w')

    cell = 200                     # 区域划分精度
    track = list()
    last_cover = list()
    for i in range(num):
        track.append(np.loadtxt(pardir + '/MAControl-AFIT/track/uav_%d_track.txt' % i))
        last_cover.append([])
    ...
    for l in range(0, np.size(track[-1], 0), 5):
        ...
        with open(pardir + '/MAControl-AFIT/cover_rate_Folder' + txt_name, 'a') as c:
...
if __name__ == '__main__':
    ...
    coverage = calculate_coverage(10, step_)
